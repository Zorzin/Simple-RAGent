...
export default async function GroupsPage({ params, searchParams }: Props) {
  ...
  const whereClause = query
    ? and(eq(groups.organizationId, organization.id), ilike(groups.name, `%${query}%`))
    : eq(groups.organizationId, organization.id);
  ...
  const [rows, totalRows] = await Promise.all([
    db.select().from(groups).where(whereClause).orderBy(orderByClause).limit(limit).offset(offset),
    db
          ...
          <table data-admin-table className="w-full border-collapse text-sm">
              ...
              <tr>
                <th className="px-4 py-3 text-left font-semibold">
                  <Link
                    ...
                    href={`/${locale}/admin/groups?sort=name&dir=${
                ...
                <th className="px-4 py-3 text-left font-semibold">
                  <Link
                    className="hover:text-zinc-900"
                    href={`/${locale}/admin/groups?sort=createdAt&dir=${
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((group) => (
                  <tr key={group.id} className="border-t border-zinc-200">
          ...
          <div className="flex items-center justify-between border-t border-zinc-200 px-6 py-3 text-xs text-zinc-500">
            ...
            <div className="flex items-center gap-2">
              <Link
                ...
                href={`/${locale}${buildPageHref("/admin/groups", {
