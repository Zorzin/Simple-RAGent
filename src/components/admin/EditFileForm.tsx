...
import type { renameFile } from "@/app/[locale]/admin/actions";

type ActionResult = Awaited<ReturnType<typeof renameFile>>;
...
export default function EditFileForm({
  ...
  const t = useTranslations("admin.filesEdit");
  ...
  return (
    <Card className="space-y-4 p-6">
      <form action={formAction} className="space-y-3" onSubmit={() => setDismissed(false)}>
        ...
        <Input name="name" defaultValue={fileName} />
        <div className="text-xs text-zinc-500">{fileMeta}</div>
