...
import type { updateTokenLimit } from "@/app/[locale]/admin/actions";

type ActionResult = Awaited<ReturnType<typeof updateTokenLimit>>;
...
export default function EditLimitForm({
  locale,
  limitId,
  userLabel,
  intervalLabel,
  ...
  const t = useTranslations("admin.limitsEdit");
  ...
  return (
    <Card className="space-y-4 p-6">
      <form action={formAction} className="space-y-3" onSubmit={() => setDismissed(false)}>
        ...
        <div className="text-xs text-zinc-500">{userLabel}</div>
        <div className="text-xs text-zinc-500">{intervalLabel}</div>
        <Input name="limitTokens" defaultValue={limitTokens} />
